...
The optional argument is a scope: `recent` (last ~90 days, default), `all` (whole vault), or a topic. This reviews the lessons P's vault has accumulated so they compound instead of expiring. It **proposes** — it never auto-prunes, auto-archives, or rewrites `CLAUDE.md` on its own.

1. Read `admin/mbs_system/brain/CLAUDE.md`, `SOUL.md`, and `admin/mbs_system/brain/LEARNINGS_DIGEST.md` + the last ~20 lines of `admin/mbs_system/design/log.md` for context.
...
5. **Suggest, don't dispose.** Only on P's explicit approval: write the report (to `admin/reviews/` like `/obsidian-review`, or a relevant project note), propose a `CLAUDE.md` / `references/` edit (propose the exact diff — never silently rewrite the operating manual), or flag a stale note for archiving (P moves it; the agent never does). If P approves writing, the note follows the amnesia test and links its source notes. Append to `admin/mbs_system/design/log.md`; note in today's tasks daily note.
