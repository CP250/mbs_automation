...
4. Save to `admin/mbs_system/design/atlas.canvas` for full scope, or `admin/mbs_system/design/atlas_<scope>.canvas` when scoped. If a file of that name exists, confirm before overwriting (do not clobber a prior map silently).
...
**Note rule:** Follows `references/ai-first-rules.md` (amnesia test) and `references/write-rules.md`. No `## For future Claude` preamble, no `ai-first:` flag — hybrid vault. The `.canvas` output lives under `admin/mbs_system/design/`, not the vault root or a `wiki/` folder. Read-only over the vault apart from the canvas file; existing human notes are never modified. Never read `trash/`. Never touch P's own sections of the daily note.
