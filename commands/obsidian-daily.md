...
2. Target file: `daily_notes/tasks/tasks_YYYY-MM-DD.md` (today, US Eastern).
   ...
   - Append the `## Vault Agent` section with an Edit that inserts after the frontmatter / existing content. Do not overwrite P's own content. If a `## Vault Agent` section already exists from an earlier run today, refresh it in place rather than adding a second one.
